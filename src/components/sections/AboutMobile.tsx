...
export function AboutMobile() {
  ...
  return (
    <section
      id="about"
      ref={ref}
      className="py-10 md:py-32 px-6 bg-secondary"
    >
      <motion.div
      ...
      >
        ...
        <div className="md:grid md:grid-cols-2 md:gap-16 md:items-center">
          <motion.div
            className="text-body text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto space-y-4 md:mb-0 md:text-lg"
            initial={{ opacity: 0 }}
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6">
            {features.map((feature, index) => (
              <motion.div
                key={feature.label}
                className="glass-card p-4 md:p-8 rounded-xl text-center hover:bg-card/80 transition-colors"
                initial={{ y: 30, opacity: 0 }}
                animate={isInView ? { y: 0, opacity: 1 } : {}}
                transition={{ duration: 0.6, delay: 0.3 + index * 0.1 }}
              >
                <feature.icon className="text-primary w-8 h-8 md:w-12 md:h-12 mx-auto mb-2 md:mb-4" />
